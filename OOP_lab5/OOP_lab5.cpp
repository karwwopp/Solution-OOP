...
;            if (!finput.is_open()) {
        ...
        case 11:
            system("cls");
            foutput.open("Students.txt");
            if (foutput.bad()) {
                cout << "Ошибка открытия файла\n";
                system("pause");
                break;
            }
            foutput << students.size() << "\n";
            for(int i = 0;i < students.size();i++){
                foutput << students[i];
            }
            cout << "Данные успешно внесены\n";
            foutput.close();
            system("pause");
            break;
        case 12:
            ...
            getline(cin, name);
            finput >> size;
            pos_begin = finput.tellg();
            finput.seekg(pos_begin);
            cout << left << setw(15) << "Имя" << setw(20) << "Возраст" << setw(20) << "Номер студ. билета\n";
            cout << left << setfill('-') << setw(55) << " " << setfill(' ') << endl;
            for (int i = 0; i < size; i++) {
                finput >> check_file_name >> age >> studentID;
                finput >> check_work;
                if (finput.fail()) {
                ...
                if (name == check_file_name) {
                    cout << setw(15) << name << setw(20) << age << setw(20) << studentID;
                }
            }
            finput.close();
            cout << "\n";
